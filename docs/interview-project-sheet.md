...
- **多 Agent 编排与局部重规划：** 基于 FloatTrip 二次开发 LangGraph 流程，以 `TravelPlanState` 管理候选池、路线和审核状态；用户修改时从 checkpoint 恢复，仅对候选不足或新增 POI 定向检索。
- **输出质量与评测：** 确定性风险门控先检查候选池越界、重复 POI、开放时间、天气和步行距离约束；低风险跳过 LLM Reviewer，高风险进入审核修复。39 条案例通过数为 **9/39、26/39、35/39**。
- **记忆、RAG 与交互：** SQLite 保存结构化偏好，Chroma 保存按用户隔离的语义记忆；知识库采用按段落切块、重叠和向量/关键词混合召回，Pydantic 约束 Agent 消息，FastAPI + SSE 流式返回节点进度。
- **工具调用与性能：** 将原直线距离替换为高德步行/驾车路线核验，并行执行天气、意图改写和多段路线查询；在 6 条保存的真实 API 响应回放中，自适应审核平均延迟较全量审核降低 **21.6%**，3 条自然请求样本 Token 估算减少 **61.8%**；小样本、仅用于机制验证，不代表线上 SLA 或实际账单。
