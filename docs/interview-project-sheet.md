...
- **多 Agent 编排与局部重规划：** 基于 FloatTrip 二次开发 LangGraph 旅行规划流程，以 `TravelPlanState` 保存候选池、路线、风险和审核状态；用户提出修改意见时从 checkpoint 恢复，复用候选池，仅在候选不足或指定新 POI 时触发定向检索。
- **输出质量与评测：** 先用确定性风险门控检查结构、重复 POI、天气和距离约束，低风险直接跳过 LLM Reviewer，高风险才进入审核修复；39 条分层案例中 Planner-only、Planner+Reviewer、完整审核链路通过数为 **9/39、26/39、35/39**。
- **记忆、RAG 与交互：** SQLite 保存结构化偏好和行程（事实源），Chroma 保存按 `user_id` 隔离的语义记忆，默认召回 Top-3；旅行知识库按约 420 字符切块并保留约 80 字符段落重叠，Chroma 向量召回与关键词召回用 RRF 融合，Pydantic 校验失败走有限重试，超限转为失败/降级状态；FastAPI + SSE 仅用于流式返回节点进度与结果。
- **工具调用与性能：** 将原直线距离判断替换为高德步行/驾车路线距离核验，并行执行天气、意图改写和多段路线查询；6 条保存的真实 API 响应回放、3 条自然请求样本仅用于机制验证，Token 为估算值，不能解释为线上 SLA 或实际账单。
