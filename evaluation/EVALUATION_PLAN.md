...
The risk-gate regression matrix additionally uses 49 hand-labelled routes over frozen AMap candidate pools across five cities. It pairs legitimate and faulty schedules, and covers duplicate/unknown POIs, day numbering, time structure, opening hours, rain, habits, road-distance constraints, user changes and compound faults. The evaluator requires the complete risk-label set to match exactly, so unexpected over-escalation is a failure. This is deterministic policy coverage, not 49 new LLM-generated plans or a statistically representative online quality estimate. The matrix found a missing day-number check and an incorrectly labelled duplicate-POI case during expansion; both were fixed. Run `python scripts/evaluate_risk_gate.py` to regenerate per-case decisions and category recall; any mismatch exits nonzero. A 100% matrix score means the deterministic policy matches its frozen labels; it is not a model accuracy or production false-positive rate.
